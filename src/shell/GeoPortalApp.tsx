...
function reducer(state: GeoPortalState, action: Action): GeoPortalState {
	switch (action.type) {
		...
		case 'setTheme': {
			return { ...state, theme: action.theme, baseMap: action.theme === 'dark' ? 'dark' : state.baseMap === 'dark' ? 'light' : state.baseMap }
		}
		case 'setBaseMap':
		...
		case 'saveDrawingsAsLayer': {
			...
			return { ...state, layers: [...state.layers, layer], drawings: { type: 'FeatureCollection', features: [] }, drawMode: 'none' }
		}
		case 'moveLayer': {
...
export function GeoPortalApp(): JSX.Element {
	const [state, dispatch] = useReducer(reducer, initialState, (s) => {
		const savedTheme = (localStorage.getItem('geoportal:theme') as 'light' | 'dark' | null) ?? s.theme
	...
	return (
		<GeoPortalContext.Provider value={ctx}>
			<div className="h-full w-full grid" style={{ gridTemplateColumns: state.sidebarOpen ? '320px 1fr' : '0 1fr' }}>
				<aside className={`relative h-full overflow-hidden border-r ${state.sidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
					<Sidebar />
					<button
				...
				<main className="h-full flex flex-col">
					...
					<div className="flex-1 relative">
						...
						<WmsDialog
							...
							onOpenChange={(o) => dispatch({ type: o ? 'openWmsDialog' : 'closeWmsDialog' })}
							onAdd={(url, names) => {
