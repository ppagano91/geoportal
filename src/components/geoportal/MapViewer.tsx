...
const INITIAL_CENTER: [number, number] = [-58.4173, -34.6118] // Buenos Aires
...
function addOrUpdateGeoJson(map: Map, layer: Layer) {
	...
	if (needRecreate) {
		...
		const ids = [pointLayerId, pointSymId, ptUnclusterCircleId, ptUnclusterSymbolId, clusterLayerId, clusterTextId, lineLayerId, polyFillId, polyLineId]
	...
	function ensureSymbolImage() {
		...
		const img = { width: base, height: base, data: ctx.getImageData(0,0,base,base).data } as any
		if (map.hasImage(imgName)) {
			try { map.updateImage(imgName, img) } catch { /* older versions may not support updateImage */ }
			return imgName
		}
	...
	if (layer.pointStyle) {
		...
		if (useCluster) {
				...
				map.addLayer({
					...
					paint: {
						...
						'circle-radius': [
							...
							50, Math.max(18, layer.pointStyle.size + 10),
							100, Math.max(24, layer.pointStyle.size + 14)
						]
					}
				})
			} else {
				map.setPaintProperty(clusterLayerId, 'circle-color', layer.pointStyle.color)
			}
			...
			map.setLayoutProperty(clusterLayerId, 'visibility', layer.visible ? 'visible' : 'none')
			map.setLayoutProperty(clusterTextId, 'visibility', layer.visible ? 'visible' : 'none')
		} else {
			if (map.getLayer(clusterLayerId)) map.setLayoutProperty(clusterLayerId, 'visibility', 'none')
			if (map.getLayer(clusterTextId)) map.setLayoutProperty(clusterTextId, 'visibility', 'none')
		}

		// unclustered or simple points
		if (isCircle) {
			// hide symbol variants
			if (map.getLayer(pointSymId)) map.setLayoutProperty(pointSymId, 'visibility', 'none')
			if (map.getLayer(ptUnclusterSymbolId)) map.setLayoutProperty(ptUnclusterSymbolId, 'visibility', 'none')
			if (useCluster) {
				if (!map.getLayer(ptUnclusterCircleId)) {
					map.addLayer({
						...
						type: 'circle',
						source: sourceId,
						filter: ['all', ['!', ['has', 'point_count']], ['in', ['geometry-type'], ['literal', ['Point', 'MultiPoint']]]],
						paint: {
					...
					map.setPaintProperty(ptUnclusterCircleId, 'circle-radius', layer.pointStyle.size)
					map.setPaintProperty(ptUnclusterCircleId, 'circle-color', layer.pointStyle.color)
					map.setPaintProperty(ptUnclusterCircleId, 'circle-stroke-color', layer.pointStyle.strokeColor)
					map.setPaintProperty(ptUnclusterCircleId, 'circle-stroke-width', layer.pointStyle.strokeWidth)
				}
				map.setLayoutProperty(ptUnclusterCircleId, 'visibility', layer.visible ? 'visible' : 'none')
				// hide simple circle
				...
				if (!map.getLayer(pointLayerId)) {
					...
					map.setPaintProperty(pointLayerId, 'circle-radius', layer.pointStyle.size)
					map.setPaintProperty(pointLayerId, 'circle-color', layer.pointStyle.color)
					map.setPaintProperty(pointLayerId, 'circle-stroke-color', layer.pointStyle.strokeColor)
					map.setPaintProperty(pointLayerId, 'circle-stroke-width', layer.pointStyle.strokeWidth)
				}
				map.setLayoutProperty(pointLayerId, 'visibility', layer.visible ? 'visible' : 'none')
				// hide uncluster alternatives
				if (map.getLayer(ptUnclusterCircleId)) map.setLayoutProperty(ptUnclusterCircleId, 'visibility', 'none')
			}
		} else {
			// symbol
			...
			if (useCluster) {
				...
				map.setLayoutProperty(ptUnclusterSymbolId, 'visibility', layer.visible ? 'visible' : 'none')
				// hide simple symbol
				if (map.getLayer(pointSymId)) map.setLayoutProperty(pointSymId, 'visibility', 'none')
			} else {
				...
				map.setLayoutProperty(pointSymId, 'visibility', layer.visible ? 'visible' : 'none')
	...
	if (layer.lineStyle) {
		if (!map.getLayer(lineLayerId)) {
			...
			map.setLayoutProperty(lineLayerId, 'line-cap', layer.lineStyle.lineCap ?? 'butt')
		}
	...
	if (layer.polygonStyle) {
		if (!map.getLayer(polyFillId)) {
			...
			map.setPaintProperty(polyFillId, 'fill-color', layer.polygonStyle.fillColor)
			map.setPaintProperty(polyFillId, 'fill-opacity', layer.polygonStyle.fillOpacity)
		}
		map.setLayoutProperty(polyFillId, 'visibility', layer.visible ? 'visible' : 'none')
		...
		if (!map.getLayer(polyLineId)) {
			...
			map.setPaintProperty(polyLineId, 'line-color', layer.polygonStyle.strokeColor)
			map.setPaintProperty(polyLineId, 'line-width', layer.polygonStyle.strokeWidth)
		}
...
function removeGeoJson(map: Map, layerId: string) {
	const ids = [`pt-${layerId}`, `ln-${layerId}`, `pf-${layerId}`, `pl-${layerId}`]
	for (const id of ids) {
		if (map.getLayer(id)) map.removeLayer(id)
	}
	const srcId = `src-${layerId}`
	if (map.getSource(srcId)) map.removeSource(srcId)
	const cache = getCfgCache(map); delete cache[layerId]
}
...
export function MapViewer(): JSX.Element {
	...
	const [popup, setPopup] = useState<{ coord: [number, number]; feature: any } | null>(null)
	const [tempFeature, setTempFeature] = useState<GeoJSON.Feature | null>(null)

	const styleUrl = useMemo(() => BASEMAPS[state.baseMap], [state.baseMap])
	...
	useEffect(() => {
		...
		try {
			map.addControl(new maplibregl.AttributionControl({ compact: true }) as any, 'bottom-left')
		} catch {}
		...
		map.on('error', () => {
			// si falla cargar el estilo remoto, intentar fallback
			if (!didFallback) {
				didFallback = true
				try {
		...
		function onContext(e: MapMouseEvent) {
			try { (e.originalEvent as MouseEvent).preventDefault() } catch {}
			const features = map.queryRenderedFeatures(e.point).filter(f => !!f.properties)
		...
		return () => {
			...
			try { map.getCanvas().removeEventListener('contextmenu', preventCtx) } catch {}
			mapRef.current = null
	...
	useEffect(() => {
		...
		if (!map.getSource('drawings-src')) {
			map.addSource('drawings-src', { type: 'geojson', data: state.drawings } as any)
		} else {
			;(map.getSource('drawings-src') as any).setData(state.drawings)
		}
		if (!map.getLayer('drawings-point')) {
			map.addLayer({ id: 'drawings-point', type: 'circle', source: 'drawings-src', filter: ['==', ['geometry-type'], 'Point'], paint: { 'circle-radius': 5, 'circle-color': '#0ea5e9', 'circle-stroke-color': '#0b87bf', 'circle-stroke-width': 1 } })
		}
		if (!map.getLayer('drawings-line')) {
			map.addLayer({ id: 'drawings-line', type: 'line', source: 'drawings-src', filter: ['==', ['geometry-type'], 'LineString'], paint: { 'line-color': '#0ea5e9', 'line-width': 2 } })
		}
		if (!map.getLayer('drawings-poly')) {
			map.addLayer({ id: 'drawings-poly', type: 'fill', source: 'drawings-src', filter: ['==', ['geometry-type'], 'Polygon'], paint: { 'fill-color': '#0ea5e9', 'fill-opacity': 0.2 } })
		}
		// temp source
		if (!map.getSource('draw-temp-src')) {
			map.addSource('draw-temp-src', { type: 'geojson', data: tempFeature ? { type: 'FeatureCollection', features: [tempFeature] } : { type: 'FeatureCollection', features: [] } } as any)
		} else {
			const data = tempFeature ? { type: 'FeatureCollection', features: [tempFeature] } : { type: 'FeatureCollection', features: [] }
			;(map.getSource('draw-temp-src') as any).setData(data as any)
		}
		if (!map.getLayer('draw-temp-line')) {
			map.addLayer({ id: 'draw-temp-line', type: 'line', source: 'draw-temp-src', filter: ['==', ['geometry-type'], 'LineString'], paint: { 'line-color': '#22d3ee', 'line-dasharray': [2,2], 'line-width': 2 } })
		}
		if (!map.getLayer('draw-temp-poly')) {
			map.addLayer({ id: 'draw-temp-poly', type: 'fill', source: 'draw-temp-src', filter: ['==', ['geometry-type'], 'Polygon'], paint: { 'fill-color': '#22d3ee', 'fill-opacity': 0.15 } })
		}
		// remove missing
		...
		function subIds(l: Layer): string[] {
			...
			ids.push(`cl-${l.id}`, `cl-t-${l.id}`, `pt-${l.id}`, `pt-sym-${l.id}`, `pt-un-${l.id}`, `pt-us-${l.id}`)
	...
	useEffect(() => {
		...
		const shouldDisableDbl = state.drawMode === 'line' || state.drawMode === 'polygon'
		try {
			if (shouldDisableDbl) map.doubleClickZoom.disable()
			else map.doubleClickZoom.enable()
		} catch {}
		let drawing: { type: 'line' | 'polygon' | 'rectangle' | 'circle'; coords: [number, number][]; start?: [number, number] } | null = null
		...
		function toFeaturePolygon(coords: [number, number][]): GeoJSON.Feature {
			const closed = coords.length > 0 && (coords[0][0] !== coords[coords.length - 1][0] || coords[0][1] !== coords[coords.length - 1][1]) ? [...coords, coords[0]] : coords
			return { type: 'Feature', geometry: { type: 'Polygon', coordinates: [closed] }, properties: {} }
		}
		function rectFrom(a: [number, number], b: [number, number]): GeoJSON.Feature {
			const minX = Math.min(a[0], b[0]), maxX = Math.max(a[0], b[0])
			const minY = Math.min(a[1], b[1]), maxY = Math.max(a[1], b[1])
			const ring: [number, number][] = [[minX,minY],[maxX,minY],[maxX,maxY],[minX,maxY],[minX,minY]]
			return toFeaturePolygon(ring)
		}
		function circleFrom(center: [number, number], edge: [number, number], steps = 64): GeoJSON.Feature {
		...
		function onClick(e: MapMouseEvent) {
			...
			if (state.drawMode === 'none' || state.drawMode === 'rectangle' || state.drawMode === 'circle') return
			if (state.drawMode === 'point') {
				ctx.dispatch({ type: 'addDrawing', feature: { type: 'Feature', geometry: { type: 'Point', coordinates: [lng, lat] }, properties: {} } })
		...
		function onDblClick() {
			...
			if (drawing.type === 'line') {
				ctx.dispatch({ type: 'addDrawing', feature: toFeatureLine(drawing.coords) })
			} else if (drawing.type === 'polygon') {
				ctx.dispatch({ type: 'addDrawing', feature: toFeaturePolygon(drawing.coords) })
			}
			drawing = null
			setTempFeature(null)
		}
	...
	return (
		<div className="absolute inset-0 overflow-hidden">
			...
			<div className="absolute bottom-3 right-3 z-10"><MiniMap styleUrl={styleUrl} /></div>
			{popup && (
				<FeaturePopup
					map={mapRef.current!}
					lngLat={popup.coord}
